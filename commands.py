...
def create_test_user():
    ...
    zhangsan = UserModel(username='张三', email='zhangsan@zlck.net', password='111111', is_staff=True, role=admin_role)

    operator_role = RoleModel.query.filter_by(name='运营').first()
    lisi = UserModel(username='李四', email='lisi@zlck.net', password='111111', is_staff=True, role=operator_role)

    inspector_role = RoleModel.query.filter_by(name='稽查').first()
    wangwu = UserModel(username='王五', email='wangwu@zlck.net', password='111111', is_staff=True, role=inspector_role)
